...
impl UpdateClient {
    ...
    pub async fn pull_update(
        ...
        auth: Option<crate::RegistryAuth>,
        insecure: bool,
    ) -> anyhow::Result<tonic::Streaming<pb::update::RegistryPullResponse>> {
    ...
    pub async fn image_install(
        &mut self,
        manifest: String,
    ) -> anyhow::Result<tonic::Streaming<pb::update::ImageInstallResponse>> {
        let request = pb::update::ImageInstallRequest { manifest };
        Ok(self
            .client
    ...
    pub async fn install_cachix(
        &mut self,
        cachix_request: pb::update::Cachix,
    ) -> anyhow::Result<tonic::Streaming<pb::update::SetGenerationResponse>> {
        Ok(self
            .client
    ...
    fn registry_credentials(
        auth: Option<crate::RegistryAuth>,
    ) -> Option<pb::update::RegistryCredentials> {
        let auth = auth?;
        let auth = match auth {
            ...
            crate::RegistryAuth::Bearer { token } => {
                pb::update::registry_credentials::Auth::Bearer(pb::update::RegistryBearerAuth {
                    token,
                })
            }
        };

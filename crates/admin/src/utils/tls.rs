...
impl CliTlsOptions {
    fn into_endpoint_tls_mode(self) -> anyhow::Result<Option<EndpointTlsMode>> {
        match self.tls_mode {
    ...
    /// Converts CLI TLS options to server TLS config.
    ///
    /// # Errors
    ///
    /// Returns an error when the selected TLS mode is missing required inputs
    /// (cert/key/CA paths for `static`, trust domain for `spiffe`).
    ...
    /// Converts CLI TLS options to client TLS config.
    ///
    /// # Errors
    ///
    /// Returns an error when the selected TLS mode is missing required inputs

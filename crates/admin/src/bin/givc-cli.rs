...
fn print_registry_pull_progress(progress: pb::update::RegistryPullProgress) {
    use pb::update::registry_pull_progress::Event;
...
async fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command {
        ...
        Commands::ImageInstall { manifest } => {
            admin.image_install(manifest).await?;
        }
    }

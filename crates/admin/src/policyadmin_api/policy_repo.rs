...
impl PolicyRepoMonitor {
    pub fn new(policy_root: impl AsRef<Path>, config_path: impl AsRef<Path>) -> Result<Self> {
        let conf = JsonNode::from_file(&config_path)
            .map_err(|e| anyhow!("policy-monitor: failed to load config file: {}", e))?;

        let url = conf.get_field(&["source", "url"]);
        let branch = conf.get_field(&["source", "ref"]);
        let destination = policy_root.as_ref().join("data");

        let interval_secs = conf
            ...
            match gix::open(&destination) {
                Ok(repo) => {
                    ...
                    if remote_url.is_some_and(|u| *u == monitor.url) {
                        ...
                        info!("policy-repo: Remote URL mismatch, re-cloning...");
                    }
                }
                Err(_) => {
                    info!("policy-repo: Failed to open existing repo, re-cloning...");
                }
            }
        }

        /* If loading failed, ensure a fresh clone */
        monitor.ensure_clone()?;
        Ok(monitor)
    }

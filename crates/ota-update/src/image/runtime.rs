...
impl Runtime {
    ...
    fn recover_partial_commit(
        ...
        let storage: Vec<_> = groups
            ...
            .filter_map(|(index, group)| {
                (group.root.is_some() || group.verity.is_some()).then_some(index)
            })
            .collect();
        if storage.len() <= 2 {
            return Ok(groups);
        ...
        let id = partial
            .iter()
            .find_map(|index| groups[*index].empty_id())
            .unwrap_or("recovery")
            .to_owned();
        let mut root = None;
        let mut verity = None;
    ...
    pub(crate) fn select_update_slot(&self, manifest: &Manifest) -> Result<SlotSelection> {
        ...
        if let Some(boot) = slots.iter().find_map(|slot| {
            slot.boot
                .as_ref()
                .filter(|_| slot.is_used() && slot.is_complete() && slot.version() == Some(&target))
        }) {
            if boot.is_default {

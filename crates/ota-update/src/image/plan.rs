...
use super::install::SignedManifest;
...
impl Plan {
    ...
    pub(crate) fn install(rt: &Runtime, m: &Manifest, source: &Path) -> anyhow::Result<Self> {
        Self::install_manifest(rt, m, source, &m.kernel.full_name(source))
    }
    ...
    pub(crate) fn install_with_uki(
        ...
        Self::install_manifest(rt, signed.manifest(), source, uki_source)
    }
...
mod tests {
    ...
    fn install_uses_explicit_private_uki_snapshot() {
        let rt = make_test_runtime();
        let m = make_test_manifest();
        let signed = SignedManifest::for_test(m);

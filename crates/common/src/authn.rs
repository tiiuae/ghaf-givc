...
impl TlsConfig {
    pub async fn from_spire_agent(spire_socket: String, trust_domain: String) -> Result<Self> {
        ...
        let endpoint_url = if spire_socket.starts_with("unix://") {
            spire_socket
        } else {
            format!("unix://{}", spire_socket)
        };
        let source = loop {
            match X509Source::builder()
                ...
                .await
            {
                ...
                Err(e) => {
                    tracing::warn!(
                        "Failed to fetch credentials from SPIRE agent, retrying in 1s... Error: {:?}",
                        e
                    );

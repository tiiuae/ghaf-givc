...
func (s *UpdateServer) ListGenerations(_ context.Context, _ *pbupdate.Empty) (*pbupdate.ListGenerationsResponse, error) {
	...
	}).Debug("update: ota-update get completed")
	if rc != 0 {
	...
	resp := &pbupdate.ListGenerationsResponse{List: make([]*pbupdate.Generation, 0, len(generations))}
	for _, item := range generations {
		resp.List = append(resp.List, &pbupdate.Generation{
...
func (s *UpdateServer) Discover(_ context.Context, request *pbupdate.RegistryDiscoverRequest) (*pbupdate.RegistryDiscoverResponse, error) {
	...
	resp := &pbupdate.RegistryDiscoverResponse{List: make([]*pbupdate.AvailableUpdate, 0, len(updates))}
	for _, item := range updates {
		resp.List = append(resp.List, &pbupdate.AvailableUpdate{
...
func (s *UpdateServer) Changelog(_ context.Context, request *pbupdate.RegistryChangelogRequest) (*pbupdate.RegistryChangelogResponse, error) {
	...
	return &pbupdate.RegistryChangelogResponse{Changelog: afterDoneOutput(stdout)}, nil
}
...
func (s *UpdateServer) ImageInstall(request *pbupdate.ImageInstallRequest, stream pbupdate.Update_ImageInstallServer) error {
	...
	return runOutputCommand(
		stream,
		args,
		func(stdout []byte, eof bool) pbupdate.ImageInstallResponse {
			return pbupdate.ImageInstallResponse{
				Finished: eof,
				Output:   stringPtr(string(stdout)),
			}
		},
		func(stderr []byte, eof bool) pbupdate.ImageInstallResponse {
			return pbupdate.ImageInstallResponse{
				Finished: eof,
...
func (s *UpdateServer) InstallCachix(request *pbupdate.Cachix, stream pbupdate.Update_InstallCachixServer) error {
	...
	return runOutputCommand(
		stream,
		args,
		func(stdout []byte, eof bool) pbupdate.SetGenerationResponse {
			return pbupdate.SetGenerationResponse{
				Finished: eof,
				Output:   stringPtr(string(stdout)),
			}
		},
		func(stderr []byte, eof bool) pbupdate.SetGenerationResponse {
			return pbupdate.SetGenerationResponse{
				Finished: eof,
				Error:    stringPtr(string(stderr)),
			}
...
func registryArgs(request interface{}) []string {
	...
	case *pbupdate.RegistryDiscoverRequest:
		args = append(args, registryCredentials(req.Credentials)...)
	case *pbupdate.RegistryChangelogRequest:
		args = append(args, registryCredentials(req.Credentials)...)
	case *pbupdate.RegistryPullRequest:
		args = append(args, registryCredentials(req.Credentials)...)
	}
	return args
}

func registryCredentials(credentials *pbupdate.RegistryCredentials) []string {
	if credentials == nil || credentials.Auth == nil {
		return nil
	}
	switch auth := credentials.Auth.(type) {
	case *pbupdate.RegistryCredentials_Basic:
		return []string{"--username", auth.Basic.Username, "--password", auth.Basic.Password}
	case *pbupdate.RegistryCredentials_Bearer:
...
func runUpdateCommand(args ...string) ([]byte, []byte, int, error) {
	...
	if err := cmd.Run(); err != nil {
		if exitErr, ok := err.(*exec.ExitError); ok {
			...
			}).Debug("update: ota-update exited nonzero")
...
func readPullProgress(reader io.ReadCloser, stream pbupdate.Update_PullServer) {
	...
	flushLine := func(line []byte) error {
		...
		if err := json.Unmarshal(line, &event); err == nil && event.Event != "" {
			...
			if resp := registryEventToProgress(event); resp != nil {
				if err := stream.Send(&pbupdate.RegistryPullResponse{
					Update: &pbupdate.RegistryPullResponse_Progress{Progress: resp},
				}); err != nil {
					return err
				}
			}
	...
	if result := parsePullResult(resultBuf.String()); result != nil {
		if err := stream.Send(&pbupdate.RegistryPullResponse{
			Update: &pbupdate.RegistryPullResponse_Result{Result: result},
		}); err != nil {
...
func registryEventToProgress(event registryEvent) *pbupdate.RegistryPullProgress {
	switch event.Event {
	case "pull_started":
		return &pbupdate.RegistryPullProgress{
			Event: &pbupdate.RegistryPullProgress_PullStarted{
				PullStarted: &pbupdate.RegistryPullStarted{
	...
	case "blob_downloading":
		progress := &pbupdate.RegistryPullProgress{
			Event: &pbupdate.RegistryPullProgress_BlobDownloading{
				BlobDownloading: &pbupdate.RegistryBlobDownloading{
					Digest:     event.Digest,
	...
	case "blob_verified":
		return &pbupdate.RegistryPullProgress{
			Event: &pbupdate.RegistryPullProgress_BlobVerified{
				BlobVerified: event.Digest,
			},
		}
	case "manifest_written":
		return &pbupdate.RegistryPullProgress{
			Event: &pbupdate.RegistryPullProgress_ManifestWritten{
				ManifestWritten: event.Path,
			},
		}
	case "cancelled":
		return &pbupdate.RegistryPullProgress{
			Event: &pbupdate.RegistryPullProgress_Cancelled{
				Cancelled: event.Stage,
			},
		}
	case "done":
		return &pbupdate.RegistryPullProgress{
			Event: &pbupdate.RegistryPullProgress_Done{
				Done: true,
			},
		}
	default:
...
func parsePullResult(stdout string) *pbupdate.RegistryPullResult {
	...
	return &pbupdate.RegistryPullResult{
		OutputDir:    outputDir,

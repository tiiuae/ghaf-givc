...
type SocketStreamClient interface {
	// Bi-directional framed socket stream with explicit EOF signalling.
	TransferData(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[StreamFrame, StreamFrame], error)
}
...
func (c *socketStreamClient) TransferData(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[StreamFrame, StreamFrame], error) {
...
type SocketStream_TransferDataClient = grpc.BidiStreamingClient[StreamFrame, StreamFrame]
...
type SocketStreamServer interface {
	// Bi-directional framed socket stream with explicit EOF signalling.
	TransferData(grpc.BidiStreamingServer[StreamFrame, StreamFrame]) error
...
func (UnimplementedSocketStreamServer) TransferData(grpc.BidiStreamingServer[StreamFrame, StreamFrame]) error {
	return status.Error(codes.Unimplemented, "method TransferData not implemented")
}
...
func RegisterSocketStreamServer(s grpc.ServiceRegistrar, srv SocketStreamServer) {
	// If the following call panics, it indicates UnimplementedSocketStreamServer was
...
func _SocketStream_TransferData_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(SocketStreamServer).TransferData(&grpc.GenericServerStream[StreamFrame, StreamFrame]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type SocketStream_TransferDataServer = grpc.BidiStreamingServer[StreamFrame, StreamFrame]

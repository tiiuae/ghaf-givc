...
func main() {
	...
	SetupEventService(agent, cfgAgent, tlsConfig)
...
func SetupEventService(agent givc_types.TransportConfig, cfgAgent *givc_types.EndpointConfig, tlsConfig *tls.Config) {
	...
	for _, eventConfig := range eventConfigs {
		...
		eventProxyServer, err := givc_eventproxy.NewEventProxyServer(eventConfig.Transport, eventConfig.Producer)
		if err != nil {
		...
		if !eventConfig.Producer {
			...
			go func(eventConfig givc_types.EventConfig) {
				...
				err = eventProxyServer.StreamEventsToRemote(context.Background(), eventClient)
				for err != nil {
